...
public class MenuService {
    ...
    public String readSearchQuery(Scanner sc, String type) {
        ...
        if (query.isEmpty()) {
            System.out.println("⚠️ Search cannot be empty.");
            return null;
        }
        return query;
    }
    ...
    public int readChoice(Scanner sc, int max) {
        while (true) {
            ...
            if (choice == 0) {
                System.out.println("↩️ Selection canceled.");
                return -1;
            }

            if (choice < 1 || choice > max) {
                System.out.println("⚠️ Choice out of range!");
                continue;
            }
    ...
    public int readRating(Scanner sc) {
        while (true) {
            ...
            try {
                ...
                System.out.println("⚠️ Invalid rating! Please enter a number from 0 to 5.");
                continue;
            }

            if (rating < 0 || rating > 5) {
                System.out.println("⚠️ Rating out of range! Please enter a number from 0 to 5.");
                continue;
            }
    ...
    public void printAudioInfo(Playable audio) {
        ...
        if (audio instanceof Music music) {
            ...
            System.out.println("⏱️ Duration: " + music.getFormattedDuration());
        } else if (audio instanceof Podcast podcast) {
            System.out.println("🎙️ Host: " + podcast.getHost());
            System.out.println("📝 Episode title: " + podcast.getEpisodeTitle());
            System.out.println("🔢 Episode: " + podcast.getEpisodeNumber());
            System.out.println("⏱️ Duration: " + podcast.getFormattedDuration());
        }
